...
// Definicje ekranow
static unsigned char code Screan[] =  "  Podejscie 00  "
																			"  Twoj typ: 00  "
																			"++++++++++++++++"
																			"                ";
...
// Funkcja aktualizaujaca licznik na wyswietlaczu
void updateDisplay(unsigned char drawCounter, unsigned char type) {
	unsigned char counterD = drawCounter / 10;
	unsigned char counterJ = drawCounter % 10;

	unsigned char typeD = type / 10;
	unsigned char typeJ = type % 10;
	
	Lcd_WriteControl(LCD_SET_DDRAM_ADDR | 12);
	Lcd_WriteData(0x30 | counterD);
	Lcd_WriteControl(LCD_SET_DDRAM_ADDR | 13);
	Lcd_WriteData(0x30 | counterJ);
	Lcd_WriteControl(LCD_SET_DDRAM_ADDR);
	
	Lcd_WriteControl(LCD_SET_DDRAM_ADDR | 0x4C);
	Lcd_WriteData(0x30 | typeD);
	Lcd_WriteControl(LCD_SET_DDRAM_ADDR | 0x4D);
	Lcd_WriteData(0x30 | typeJ);
	Lcd_WriteControl(LCD_SET_DDRAM_ADDR);
}
